...
function createCreationUpdate(data) {
  const eventData = {};
  eventData.symbol = data.s;
  eventData.event = 'creations-updates';
  eventData.id = data.c;
  eventData.side = data.S.toLowerCase();
  eventData.price = +data.p;
  eventData.quantity = +data.q;
...
function createExecution(data) {
  ...
  eventData.id = data.c;
  eventData.side = data.S.toLowerCase();
  eventData.price = +data.L;
  eventData.quantity = +data.l;
  eventData.timestamp = moment(+data.T).utc().format('YYYY-MM-DD HH:mm:ss.SSS');
  return eventData;
};
...
function Ws(wsSettings = {}) {
   ...
   * 
   */
  wsSettings.URL = wsSettings.URL || 'wss://stream.binance.com';
  /** 
   * 
   * 
   * REST
  ...
  const rest = Rest({
    ASSET_SPOT: wsSettings.ASSET_SPOT,
  ...
  const ordersOnMessage = (message) => {
    const messageParse = JSON.parse(message);
    if (messageParse.e !== 'executionReport') { return };
  ...
  const positionsOnMessage = (message) => {
    const messageParse = JSON.parse(message);
    if (messageParse.e !== 'outboundAccountPosition') { return };
    messageParse.B.forEach(positionEvent => {
      const positionData = positionsWsObject.data[0];
      if (!positionData) { return };
      if (positionEvent.a !== wsSettings.ASSET_SPOT) { return };
      positionData.pxS = 0;
      positionData.pxB = 0;
      positionData.qtyS = +positionEvent.f + +positionEvent.l;;
      positionData.qtyB = 0;
    });
