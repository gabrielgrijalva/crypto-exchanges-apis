...
function Ws(wsSettings = {}) {
  ...
  webSocket.addOnClose(async () => {
    ...
    for (const params of markPricesOptionsWsObject.subscriptions) await markPricesOptionsWsObject.subscribe(params);
  });
  ...
  const liquidationsOnMessagePortfolio = (message) => {
    ...
    liquidationsWsObject.data.forEach(liquidationData => {
      ...
      liquidationData.liqPxS = liquidationData.qtyS ? +portfolioEvent.estimated_liquidation_ratio_map[`${liquidationSubscription.asset.toLowerCase()}_usd`] * liquidationData.markPx : 0;
      liquidationData.liqPxB = liquidationData.qtyB ? +portfolioEvent.estimated_liquidation_ratio_map[`${liquidationSubscription.asset.toLowerCase()}_usd`] * liquidationData.markPx : 0;
    });
  };
